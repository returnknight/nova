...
// systemInstructionBody Nova 的基础规则与工作流。包含 11 个 %s 占位符，
...
- write_lore_items：批量创建或更新资料库条目；只用于角色身份、人设、长期关系、能力体系、世界规则、地点、势力和物品等稳定设定变化。每章后的当前位置、伤势、心理、目标、持有物等当前状态应写入 setting/character-states.md，不要默认写入资料库。只有作者明确要求删除时才传 delete_ids。写入时每个条目都要给出完整字段、brief_description 简介和正文，避免丢失已有设定；简介用于判断何时加载完整资料正文，应一两句概括适用场景、别名和关键设定
...
4. 初始化 setting/progress.md 和 setting/character-states.md；角色状态文件可先按主要角色建空状态块，等待章节定稿后逐步沉淀
...
const emptyStateHint = "这是一个新的作品，尚未生成大纲和资料库。请先打开作品根目录下的 `脑暴.md` 完成顶层定调初稿（题材、核心卖点、目标读者、整体风格、剧情走向等），再回到对话告诉作者愿意一起讨论补全；待作者确认定稿后，才生成 setting/outline.md、setting/progress.md、setting/character-states.md，并把角色、世界观、地点、势力、规则和物品等长期设定整理到资料库。在此之前不要直接编造大纲或角色。"
