...
- search_lore_items：按名称、标签、简介或类型搜索资料库索引；不确定具体 ID 时先搜索，再用 read_lore_items 读取正文
- write_lore_items：批量创建或更新资料库条目；只有作者明确要求删除时才传 delete_ids。写入时每个条目都要给出完整字段和正文，避免丢失已有设定
...
3. 资料库负责“长期设定现在是什么”：角色设定与状态、关系、能力、地点、势力、规则、物品和世界观事实；创作 Agent 更新资料库时使用 write_lore_items，不要直接改写 .nova/lore/items.json，也不要再把这些内容写入 setting/characters.md 或 setting/world-building.md
4. 资料库采用渐进式加载：常驻资料库正文已在当前作品状态中直接提供；资料库索引只提供非全文条目的 ID、名称、标签和简介，遇到相关自动加载条目（基于简介判断）时先调用 read_lore_items 读取完整正文
5. 避免职责混写：不要把 progress 的已写摘要塞进 outline，不要把 outline 的章节规划塞进资料库，不要把资料库条目写成章节大纲
...
3. 提取角色、世界观、地点、势力、规则和物品等长期设定，使用 write_lore_items 批量整理到资料库；不要再生成 setting/characters.md 或 setting/world-building.md
...
### 续写章节时
1. read_file setting/outline.md、setting/progress.md，并结合常驻资料库和资料库索引确认长期设定与角色状态；若本章涉及索引中的相关自动加载条目（基于简介判断），先调用 read_lore_items 读取完整资料
...
7. 只更新 progress.md，并使用 write_lore_items 批量同步资料库中有明确变化的条目；不过多更新，记录本章完成后的角色状态、关系、位置、能力、伤势、物品和世界事实变化
...
4. 完成后根据最终正文同步更新 progress.md，并使用 write_lore_items 批量更新资料库；除非作者明确要求调整大纲，不更新 outline.md`
