...
- WebUI 互动模式：故事舞台的可选择候选与输入框改为嵌入底部布局，不再悬浮遮挡正文，并增强 `<HOT_STATE>` 流式过滤避免隐藏状态泄露到主屏幕
...
- WebUI/后端资料库：支持导入 SillyTavern 酒馆 v2 角色卡，PNG `chara` 元数据与 JSON 角色卡均可上传；后端会按互动资料库 Lore Item 格式写入 `.nova/lore/items.json`，角色主体导入为角色资料，角色卡附带世界书条目导入为资料库世界观条目；入口从 IDE 目录栏移动到与书籍管理并列的全局活动栏，并可在导入弹窗中选择导入到当前书籍或用角色卡名称创建一本新书后导入
