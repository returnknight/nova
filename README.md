...
- `chapters/` 存放正文，章节文件建议遵循 `chXX-章节名.md`，例如 `ch01-废材开局.md`。
- `drafts/` 是可选草稿目录；默认草稿流程关闭，只有启用草稿流程或明确要求草稿时使用。
- `setting/styles/` 存放风格参考样本，只在本轮通过 `#` 指定或命中场景化风格规则时注入 Agent。
...
2. 让 Agent 生成或调整 `setting/outline.md`、`setting/characters.md`、`setting/progress.md`。
...
- 左侧互动设置区可维护资料库、创作者指令和讲述者规则。
- 互动回复长度和舞台字号等参数可在设置页的「互动模式」分组调整。
...
- 工作区级适合当前书籍的章节命名、章节组建议规模、草稿流程、最大打开 Tab、场景化风格规则、互动模式字数等。
